      ...
      <div className="max-w-xl mx-auto bg-black text-white min-h-screen">
        ...
        <div className="border-b border-gray-800 p-4">
          ...
          <div>
              ...
              <div className="text-gray-500">@{post.author?.username.toLowerCase().replace(/\s+/g, '')}</div>
            </div>
          </Link>
          </div>
          ...
          <div className="text-gray-500 text-sm py-3 border-t border-b border-gray-800">
            {new Date(post.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} · {new Date(post.createdAt).toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'})}
          </div>
                ...
                <span className="font-bold">
                  {
                    ...
                    (post.comments?.reduce((acc, comment) => acc + (comment.replies?.length || 0), 0) || 0)
                  }
                </span>
          ...
          <div className="flex justify-around py-2 border-b border-gray-800">
            ...
            >
              <FaHeart size={18} className="mr-2" fill={post.likes?.includes(user?._id) ? "#ec4899" : "gray"} color={post.likes?.includes(user?._id) ? "#ec4899" : "currentColor"} />
        ...
        <div className="px-4 py-2">
          ...
          <div className="flex items-center border-b border-gray-800 pb-4 mb-2">
            ...
            <div className="flex-1 relative">
              ...
              <button
                ...
                className={`absolute right-2 top-2 ${comment.trim() ? 'text-blue-400 hover:bg-blue-900/20' : 'text-gray-600'} p-1 rounded-full`}
              >
                <IoMdSend size={16} />
              </button>
            </div>
          ...
          <div className="flex">
            <div className="w-10 h-10 rounded-full mr-3">
              <img src={`http://localhost:5000${c.user?.profilePicture}`} className="w-10 h-10 rounded-full object-cover ogComment" />
            </div>
            <div className="flex-1">
              <Link to={`/users/${c.user._id}`}>
              <div className="flex items-center">
                <span className="font-bold mr-2">{c.user?.username}</span>
                <span className="text-gray-500">@{c.user?.username?.toLowerCase().replace(/\s+/g, '')}</span>
              </div>
              </Link>
              ...
              <div className="flex text-sm text-gray-400 mt-1 space-x-4">
                <button onClick={() => handleCommentLike(c._id)} className="hover:text-pink-500">
                  <FaHeart size={14} className="inline mr-1" fill={c.likes?.includes(user._id) ? "#ec4899" : "gray"} />
                  {c.likes?.length || 0}
                </button>
                <button onClick={() => setActiveReplyId(activeReplyId === c._id ? null : c._id)} className="hover:text-blue-400">
                  Reply
                </button>
                {c.replies?.length > 0 && (
                  <button 
                  ...
                  >
                    {expandedReplies[c._id] ? (
                      <><FaChevronDown size={12} className="mr-1" /> Hide {c.replies.length} {c.replies.length === 1 ? 'reply' : 'replies'}</>
                    ) : (
                      <><FaChevronRight size={12} className="mr-1" /> Show {c.replies.length} {c.replies.length === 1 ? 'reply' : 'replies'}</>
                    )}
                  </button>
                )}
              </div>
                  ...
                  {c.replies.map(reply => (
                    <div key={reply._id} className="flex">
                      <div className="w-8 h-8 rounded-full mr-2">
                        <img src={`http://localhost:5000${reply.user?.profilePicture}`} className="w-8 h-8 rounded-full object-cover nestedComment" />
                      </div>
                      <div className="flex-1">
                        <Link to={`/users/${reply.user._id}`}>
                        <div className="text-sm font-bold">{reply.user?.username}</div>
